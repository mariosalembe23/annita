      ...
      {isPending ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="h-48 bg-zinc-50 border border-zinc-200 animate-pulse rounded-2xl"
            />
          ))}
        </div>
      ) : reports.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-zinc-200 rounded-2xl bg-zinc-50/20">
          <RiFlagLine className="size-16 text-zinc-300 mb-3 mx-auto" />
        ...
        <div className="grid grid-cols-1  gap-6">
          {reports.map((report) => {
            return (
              <div
                key={report.id}
                className="bg-white border border-zinc-200 border-l-4 border-l-red-500 p-5 shadow-xs flex items-center justify-between transition-shadow"
              >
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="text-base transition-all hover:opacity-75 bg-white text-black border-gray-200 border rounded-lg px-3 py-1 font-normal  flex items-center gap-2 "
                  >
                    Detalhes
                  </button>
      ...
      {meta && meta.totalPages > 1 && (
        <div className="flex items-center justify-between pt-4 border-t border-zinc-200">
          <button
            ...
            disabled={page === 1}
            className="text-sm px-4 py-2 border rounded-lg bg-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-zinc-50 transition-colors cursor-pointer"
          >
            Anterior
          </button>
